...
CREATE TABLE mirs_movie(
  ...
  `stills_link` VARCHAR(150) COMMENT '剧照页面链接',
  `stills_photos_links` JSON COMMENT '剧照照片集合页面链接',
  `poster_link` VARCHAR(150) COMMENT '海报页面链接',
  `poster_photos_links` JSON COMMENT '海报照片集合页面链接',
  `wallpaper_link` VARCHAR(150) COMMENT '壁纸页面链接',
  ...
  `all_questions_link` VARCHAR(150) COMMENT '关于这部电影所有问题的页面链接',
  `reviews` JSON COMMENT '几个影评',
  `reviews_link` VARCHAR(150) COMMENT '所有影评页面链接',
  `short_pop_comments` JSON COMMENT '几个最新短评',
  `short_new_comments` JSON COMMENT '几个热门短评',
  `short_comments_link` VARCHAR(150) COMMENT '所有短评页面链接',
  PRIMARY KEY (id),
  INDEX idx_id(id),
