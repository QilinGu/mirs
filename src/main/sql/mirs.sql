...
CREATE TABLE mirs_register_session(
  ...
  `rgst_state` TINYINT NOT NULL DEFAULT 0 COMMENT '注册会话状态：0：正在注册;1：注册超时;2注册失败;4:注册成功',
  `rgst_client_ip` VARCHAR(15) COMMENT '客服端IP地址',
...
CREATE TABLE mirs_email_verify(
  ...
  `vrf_expire_time` DATETIME NOT NULL COMMENT '验证过期时间',
...
CREATE TABLE mirs_user(
  ...
  `u_username` VARCHAR(100) NOT NULL UNIQUE COMMENT '用户名',
  `u_password` VARCHAR(128) COMMENT '密码',
  `u_salt` VARCHAR(128) COMMENT '盐值',
  ...
  PRIMARY KEY (u_id),
  INDEX idx_user_name(u_username),
