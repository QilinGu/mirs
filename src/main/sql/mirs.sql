...
CREATE TABLE mirs_register_session(
  ...
  `status` CHAR(1) NOT NULL DEFAULT '0' COMMENT '注册会话状态：0：正在注册;1：注册超时;2注册失败;4:注册成功',
...
CREATE TABLE mirs_email_verify(
  ...
  `channel` CHAR(1) NOT NULL DEFAULT '0' COMMENT '验证渠道，默认为通过网站版验证',
  `verify_code` VARCHAR(10) NOT NULL COMMENT '验证码',
  `verify_type` CHAR(1) NOT NULL COMMENT '验证类型：0：注册;1：找回密码;2:异地登录',
  `request_ip` VARCHAR(15) COMMENT '请求IP',
  `status` CHAR(1) NOT NULL DEFAULT '0' COMMENT '验证状态：0:正在验证;1:验证超时;2:验证失败;3:验证成功',
