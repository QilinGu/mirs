...
CREATE TABLE mirs_register_session(
  ...
  `status` TINYINT NOT NULL DEFAULT 0 COMMENT '注册会话状态：0：正在注册;1：注册超时;2注册失败;4:注册成功',
...
CREATE TABLE mirs_email_verify(
  ...
  `status` TINYINT NOT NULL DEFAULT 0 COMMENT '验证状态：0:正在验证;1:验证超时;2:验证失败;3:验证成功',
...
CREATE TABLE mirs_user(
  ...
  `status` TINYINT NOT NULL DEFAULT 0 COMMENT '用户状态：0：正常;1:停用',
